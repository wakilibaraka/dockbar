    ...
    private func publishWindows(currentWindowOrder: [String]? = nil, forceDerivedState: Bool = true) {
        ...
        if didChangePublishedWindowState {
            if settings.quitOnClose {
                let currentPIDs = Set(windows.map { $0.pid })
                let nextPIDs = Set(nextWindows.map { $0.pid })
                let closedPIDs = currentPIDs.subtracting(nextPIDs)
                
                for pid in closedPIDs {
                    // Only terminate if the app is still running (hasn't already quit itself)
                    if let app = NSRunningApplication(processIdentifier: pid) {
                        // Don't quit Finder or system apps like Dock
                        if app.bundleIdentifier != "com.apple.finder" && app.bundleIdentifier != "com.apple.dock" {
                            app.terminate()
                        }
                    }
                }
