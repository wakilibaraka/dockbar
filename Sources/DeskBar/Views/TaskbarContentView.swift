...
final class TaskbarContentView: NSView {
    ...
    private func taskButtonView(
        ...
        let buttonView = TaskButtonView(
            ...
            let isActive = self?.isWindowActive(windowInfo, frontmostPID: frontmostPID, frontmostWindowID: frontmostWindowID) ?? false
            self?.handleTaskButtonClick(windowInfo: windowInfo, isActive: isActive)
        }
    ...
    private func groupedTaskView(
        ...
        case .group(let group):
            ...
            let isGroupActive = group.windows.contains {
                isWindowActive($0, frontmostPID: frontmostPID, frontmostWindowID: frontmostWindowID)
            }
            let groupView = TaskZoneGroupContainerView(
                ...
                activationHandler: { [weak self] in
                    self?.handleGroupButtonClick(group: group, isActive: isGroupActive)
                },
                windowActivationHandler: { [weak self] windowInfo in
                    self?.handleTaskButtonClick(windowInfo: windowInfo, isActive: self?.isWindowActive(windowInfo, frontmostPID: frontmostPID, frontmostWindowID: frontmostWindowID) ?? false)
                }
            )
    ...
    private func handleTaskButtonClick(windowInfo: WindowInfo, isActive: Bool) {
        if isActive {
            if settings.frontmostAppClickBehavior == .minimize {
                if let app = NSWorkspace.shared.runningApplications.first(where: { $0.processIdentifier == windowInfo.pid }) {
                    minimize(windowInfo: windowInfo, application: app)
                }
            } else {
                if let app = NSWorkspace.shared.runningApplications.first(where: { $0.processIdentifier == windowInfo.pid }) {
                    cycleWindows(for: app, currentWindow: windowInfo)
                }
            }
        } else {
            activate(windowInfo: windowInfo)
        }
    }

    private func handleGroupButtonClick(group: AppGroup, isActive: Bool) {
        guard let pid = group.windows.first?.pid else { return }
        
        if isActive {
            if settings.frontmostAppClickBehavior == .minimize {
                if let app = NSWorkspace.shared.runningApplications.first(where: { $0.processIdentifier == pid }) {
                    for window in group.windows {
                        minimize(windowInfo: window, application: app)
                    }
                }
            } else {
                if let app = NSWorkspace.shared.runningApplications.first(where: { $0.processIdentifier == pid }) {
                    // Try to cycle among the group's windows
                    if let frontmostWindowID = NSWorkspace.shared.frontmostApplication.flatMap({ app in
                        windowManager.windows.first(where: { $0.pid == app.processIdentifier && !($0.isMinimized) })?.cgWindowID
                    }) {
                        let activeWindow = group.windows.first { $0.cgWindowID == frontmostWindowID }
                        cycleWindows(for: app, currentWindow: activeWindow)
                    } else {
                        cycleWindows(for: app, currentWindow: nil)
                    }
                }
            }
        } else {
            // Activate the first or main window of the group
            if let window = group.windows.first(where: { !$0.isMinimized }) ?? group.windows.first {
                activate(windowInfo: window)
            }
        }
    }

    private func minimize(windowInfo: WindowInfo, application: NSRunningApplication) {
        guard let windowElement = matchingWindowElement(for: windowInfo, application: application) else { return }
    ...
    private func cycleWindows(for application: NSRunningApplication, currentWindow: WindowInfo?) {
        let windows = windowManager.windows.filter { $0.pid == application.processIdentifier }
        guard windows.count > 1 else { return }
        
        let currentIndex = windows.firstIndex { $0.cgWindowID == currentWindow?.cgWindowID } ?? -1
        let nextIndex = (currentIndex + 1) % windows.count
        let nextWindow = windows[nextIndex]
        
        activate(windowInfo: nextWindow)
    }
