...
final class TaskbarContentView: NSView {
    ...
    private var dockWidgetFixedWidth: CGFloat {
        ...
        guard !widths.isEmpty else { return 1 }
        return widths.reduce(0, +) + 1 + fixedWidgetSpacing * CGFloat(widths.count + 1)
    }
    ...
    private var dockWidgetWidths: [CGFloat] {
        ...
        return settings.taskbarMode.strategy.dockWidgetWidths(originalWidths: widths, clusterWidth: clusterWidth)
    }
    ...
    private func addOrderedDockWidgets() {
        ...
        for widgetID in settings.dockWidgetOrder {
            if let id = DockWidgetID(rawValue: widgetID), let view = viewsByID[id], view.superview == nil {
                if let container = settings.taskbarMode.strategy.container(
                    for: widgetID, 
                    zonesStackView: zonesStackView, 
                    windowsTrayClusterView: windowsTrayClusterView
                ) {
                    if let stack = container as? NSStackView {
                        stack.addArrangedSubview(view)
                        stack.setCustomSpacing(4, after: view)
                    } else if let tray = container as? WindowsTrayClusterView {
                        tray.addWidget(view)
                    }
                }
            }
    ...
    private func groupedTaskItems(from windows: [WindowInfo]) -> [TaskZoneItem] {
        ...
        let combinePinned = strategy.combinesPinnedApps
        ...
        return groups.map { group in
            if !strategy.groupsSingleWindows && group.windowCount == 1, let window = group.windows.first {
        ...
        TaskButtonPluginMenuConfiguration(
            ...
            menuProvider: { [weak self] in
                SMPluginAgentMenuFactory.makeMenu(
                    ...
                    showsTokenUsage: self?.settings.showSessionManagerTokenUsage ?? false
                )
            }
        )
    }
    ...
    private func shouldGroupWindows(_ windows: [WindowInfo]) -> Bool {
        ...
        return settings.taskbarMode.strategy.shouldGroupWindows(defaultGrouping: defaultGrouping)
    }
    ...
    private func handleGroupClick(_ group: AppGroup) {
        ...
        if isActive {
            settings.taskbarMode.strategy.handleGroupClick(
                group: group,
                isActive: isActive,
                app: app,
                firstWindow: firstWindow,
                accessibilityService: accessibilityService,
                defaultHide: { app.hide() }
            )
        } else {
