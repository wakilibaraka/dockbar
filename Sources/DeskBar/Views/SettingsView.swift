...
final class SettingsView: NSView {
    ...
    private func configureLayout() {
        ...
        frontmostAppClickBehaviorPopupButton.addItems(withTitles: ["Cycle Windows", "Minimize"])
        ...
        behaviorTab.view = makeFormView(rows: [
            ...
            makeLabeledControlRow(label: "Click frontmost app", control: frontmostAppClickBehaviorPopupButton),
    ...
    private func configureActions() {
        ...
        frontmostAppClickBehaviorPopupButton.target = self
        frontmostAppClickBehaviorPopupButton.action = #selector(frontmostAppClickBehaviorChanged(_:))

        quitOnCloseCheckbox.target = self
        quitOnCloseCheckbox.action = #selector(quitOnCloseChanged(_:))
    ...
    private func bindSettings() {
        ...
        settings.$frontmostAppClickBehavior
            .receive(on: RunLoop.main)
            .sink { [weak self] value in
                let index: Int
                switch value {
                ...
                self?.frontmostAppClickBehaviorPopupButton.selectItem(at: index)
            }
            .store(in: &cancellables)
        ...
        settings.$quitOnClose
            .receive(on: RunLoop.main)
            .sink { [weak self] value in
                self?.quitOnCloseCheckbox.state = value ? .on : .off
            }
            .store(in: &cancellables)
    ...
    @objc
    private func frontmostAppClickBehaviorChanged(_ sender: NSPopUpButton) {
        switch sender.indexOfSelectedItem {
        case 1:
            settings.frontmostAppClickBehavior = .minimize
        default:
            settings.frontmostAppClickBehavior = .cycleWindows
        }
    }

    @objc
    private func quitOnCloseChanged(_ sender: NSButton) {
        settings.quitOnClose = sender.state == .on
    }
